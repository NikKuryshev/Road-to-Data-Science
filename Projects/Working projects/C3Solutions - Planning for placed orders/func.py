...
def get_data(path, sheets):
    ...
    for sheet in sheets:
        ...
        df = dict.dropna(subset= ['Артикул','Количество'])#dict.loc[((dict['Артикул'] != np.nan) & (dict['Количество' ] != np.nan))]  #dict.dropna().loc[~dict['Артикул'].isin(drop_item)] # Удаление сервисных артикулов
